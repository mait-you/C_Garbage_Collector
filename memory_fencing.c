...
/*   Updated: 2025/04/19 16:54:44 by mait-you         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @param size Total size of the user's requested memory
 * @return int 1 if corrupted, 0 if intact
 */
...
void	*allocate_ptr_memory_fencing(size_t size[2], void **ptr_array)
{
	...
	ptr = ft_calloc(1, (size[0] * size[1]) + (GUARD_SIZE * 2));
	if (!ptr)
	{
...
void	*free_all_memory_fencing(void **ptr_array, size_t size)
{
	...
	while (i < MAX_ALLOCATIONS)
	{
		if (ptr_array[i])
		{
			if (check_memory_fencing(ptr_array[i], size))
				ft_putendl_fd(
	...
	size_t size)
{
	...
	i = 0;
	original_ptr = (void *)((unsigned char *)to_delete - GUARD_SIZE);
	while (i < MAX_ALLOCATIONS)
	{
		if (ptr_array[i] == original_ptr)
		{
			if (check_memory_fencing((void *)to_delete, size))
				ft_putendl_fd(
					"Error: Memory corruption detected", STDERR_FILENO);
			free(original_ptr);
			return (ptr_array[i] = NULL);
		}
		else if (ptr_array[i] == NULL && original_ptr == ptr_array[i])
		{
			ft_putendl_fd(
